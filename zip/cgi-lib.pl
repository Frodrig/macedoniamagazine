...
# $Id: cgi-lib.pl,v 2.11 1996/06/11 21:22:14 brenner Exp $
#
...
$cgi_lib'version = sprintf("%d.%02d", q$Revision: 2.11 $ =~ /(\d+)\.(\d+)/);
  ...
  if (!defined $meth || $meth eq '' || $meth eq 'GET' || 
    ...
    if (!defined $meth || $meth eq '') {
      ...
      $in = $ENV{'QUERY_STRING'};
        ...
        if (($got = sysread(STDIN, $in, $len) != $len))
...
$errflag = !(eval <<'END_MULTIPART');
   ...
   PART: # find each part of the multi-part while reading data
    while (1) {
      ...
      $errflag = (($got = sysread(STDIN, $buf, $amt, length($buf))) != $amt);
     ...
     BODY: 
      while (($bpos = index($buf, $boundary)) == -1) {
        ...
        $errflag = (($got = sysread(STDIN, $buf, $amt, $maxbound)) != $amt);  
	die "Short Read: wanted $amt, got $got\n" if $errflag;
        $left -= $amt;
      }
